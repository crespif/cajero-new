...
export default function SelectSumin({ clientes, facturas }: { clientes: Cliente[]; facturas: Factura[] }) {
  ...
  const [facts, setFacts] = useState(facturas.filter(factura => factura.CuentaNIS === client.CuentaNIS && ![9].includes(new Date(factura.FacturaFV).getMonth() + 1)));
  ...
  const handleChange = (e: any) => {
    ...
    setFacts(facturas.filter(factura => factura.CuentaNIS === selected[0].CuentaNIS && ![8].includes(new Date(factura.FacturaFV).getMonth() + 1)));
